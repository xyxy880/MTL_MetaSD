import train
import test
# import traindp as train
# import testdp as test
from utils import *
...
def build_parser(num,mode,data,flag):
    ...
    parser.add_argument('--gpu',dest='gpu_num', help='GPU Number',metavar='GPU_NUM', default='6')
...
def main():
    ...
    for data in datas:
        ...
        # TF_RECORD_PATH = ['/hdd/tianchuan/Meteorological_data/era5hourUSA/{}/Finetune.tfrecord'.format(data)]
        # NUM_OF_DATA = 720
        ...
        # TF_RECORD_PATH = '/hdd/tianchuan/Meteorological_data/era5_test_wrf/wind/finetune.tfrecord'
        # NUM_OF_DATA = 744
        ...
        if options.is_train == True:
            ...
            # model_path = '/hdd/tianchuan/Climate_work/MZSR-USA/Retrain/SR/Model{}/model-{}'.format(options.trial,300)#zhh
            ...
            for i in range(len(gt_path)):
                ...
                _, pp,ss,rr = Tester(img, gt, gt_path[i],mode,data,'mtl',i)
                P.append(pp)
                S.append(ss)
                R.append(rr)
            t2 = time.time()
