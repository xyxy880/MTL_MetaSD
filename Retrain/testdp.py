...
from Models import deepsd as model
import time
from utils import *
...
class Test(object):
    ...
    def __call__(self, img, gt, img_name,mode,v,m,idx):
        self.img = img
        ...
        # Train the network
        self.quick_test(mode,v,m,idx)

        print('[*] Baseline ')
        # self.train()

        # post_processed_output = self.final_test()
        post_processed_output = self.quick_test(mode,v,m,idx)

        if self.save_results:
            if not os.path.exists('%s' % (self.save_path)):
                os.makedirs('%s' % (self.save_path))

            imageio.imsave('%s/%s.png' % (self.save_path, os.path.basename(self.img_name)[:-4]),
                           post_processed_output)

        print('** Done Adaptation for X', self.scale, os.path.basename(self.img_name), ', PSNR: %.4f' % self.psnr[-1],
              ', SSIM: %.4f' % self.ssim[-1],  ', RMSE: %.4f' % self.rmse[-1],' **')
        print('')

        return post_processed_output, self.psnr,self.ssim,self.rmse

    def train(self):
        img = modcrop(self.img, int(self.scale))
        self.hr_father = img
        self.lr_son = imresize(img, scale=1 / self.scale, kernel=self.kernel, ds_method=self.ds_method)
    ...
    def forward_pass(self, input, output_shape=None):
        ...
        # feed_dict = {self.input: input[None, :, :, :]}
    ...
    def forward_backward_pass(self, input, hr_father):
        ...
        # feed_dict = {self.input: input[None, :, :, :], self.label: HR, self.lr_decay: self.learning_rate}
    ...
    def quick_test(self,mode,v,m,idx):
        # 1. True MSE
        ...
        if idx in [0,23,43,65,79]:
            image.imsave('/hdd/tianchuan/zhh_bias/LR/{}_{}_{}_{}'.format(mode,v,m,idx)+'.png',self.img)
            image.imsave('/hdd/tianchuan/zhh_bias/SR/{}_{}_{}_{}'.format(mode,v,m,idx)+'.png',self.sr)
            image.imsave('/hdd/tianchuan/zhh_bias/HR/{}_{}_{}_{}'.format(mode,v,m,idx)+'.png',self.gt)

        '''Shave'''
        scale = int(self.scale)
        PSNR =psnr_rgb(np.round(np.clip(self.gt*255., 0.,255.)).astype(np.uint8),
                       np.round(np.clip(self.sr*255., 0.,255.)).astype(np.uint8))

        # PSNR=psnr(rgb2y(np.round(np.clip(self.gt*255., 0.,255.)).astype(np.uint8)),
        #            rgb2y(np.round(np.clip(self.sr*255., 0., 255.)).astype(np.uint8)))
        # print(PSNR,PSNR2)
        self.psnr.append(PSNR)
        SSIM = calculate_ssim(np.round(np.clip(self.gt * 255., 0., 255.)),
                              np.round(np.clip(self.sr * 255., 0., 255.)))
        # SSIM = cal_ssim(np.round(np.clip(self.gt*255., 0.,255.)).astype(np.uint8),
        #                 np.round(np.clip(self.sr*255., 0.,255.)).astype(np.uint8),
        #                 multichannel=True)
        self.ssim.append(SSIM)
        #zhh
        RMSE=mse(np.round(np.clip(self.gt*255., 0.,255.)).astype(np.uint8),
                       np.round(np.clip(self.sr*255., 0.,255.)).astype(np.uint8))**0.5
        self.rmse.append(RMSE)
#zhh


        # 2. Reconstruction MSE
        # img = modcrop(self.img, scale)
        # print(img.shape)
        # self.reconstruct_output = self.forward_pass(self.hr2lr(img), img.shape)
        # self.mse_rec.append(np.mean((img - self.reconstruct_output) ** 2))

        processed_output = np.round(np.clip(self.sr * 255, 0., 255.)).astype(np.uint8)

        # print('iteration: ', self.iter, 'recon mse:', self.mse_rec[-1], ', true mse:',
        #       (self.mse[-1] if self.mse else None), ', PSNR: %.4f' % PSNR, ', SSIM: %.4f' % SSIM)
        # print('true mse:', (self.mse[-1] if self.mse else None), ', PSNR: %.4f' % PSNR, ', SSIM: %.4f' % SSIM)
