...
def generate_TFRecord(label_path1, label_path2, tfrecord_file, patch_h, patch_w, stride):
    a = np.sort(np.asarray(glob.glob(label_path1)))
    b = np.sort(np.asarray(glob.glob(label_path2)))
    label_list = np.concatenate([a, b])
    ...
    for n in range(fileNum):
        ...
        for i in range(0 + offset, x - patch_h + 1, 96):
            for j in range(0 + offset, y - patch_w + 1, 120):
                ...
                patch_d = lr[i // scale:i // scale + 21 // scale, j // scale:j // scale + 21 // scale]
...
if __name__ == '__main__':
    ...
    for var in vars:
        ...
        parser.add_argument('--tfrecord', dest='tfrecord', help='Save path for tfrecord file',
                            default='/hdd/tianchuan/Meteorological_data/era5hourUSA/{}_train'.format(var))
