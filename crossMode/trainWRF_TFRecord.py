...
def generate_TFRecord(data_path,label_path,tfrecord_file,patch_h,patch_w,stride):
    ...
    for n in range(fileNum):
        ...
        x, y, ch = label.shape
        lr, hr = random_crop(img, label, 67, 3)
...
if __name__=='__main__':
    ...
    # TASK=['GRDFLX','HFX','QFX','SST','LH']
    TASK=['Wind','TSK','T2','SST','LH']
    for data in TASK:
        print(data)
        parser = ArgumentParser()
        parser.add_argument('--scale', dest='scale', help='Scaling Factor for Super-Resolution', type=int, default=3)
        parser.add_argument('--labelpath', dest='labelpath', default='/hdd/tianchuan/Meteorological_data/wrf_era5/wrf/{}/HR_train'.format(data))
        parser.add_argument('--datapath', dest='datapath', default='/hdd/tianchuan/Meteorological_data/wrf_era5/wrf/{}'.format(data))
        parser.add_argument('--tfrecord', dest='tfrecord', default='/hdd/tianchuan/Meteorological_data/wrf_era5/wrf/{}/'.format(data))
        ...
        tfrecord_file = options.tfrecord + '{}_train.tfrecord'.format(data)
