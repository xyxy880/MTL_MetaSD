    ...
    for n in range(fileNum):
        ...
        for i in range(0+offset,x-patch_h+1,stride):
            for j in range(0+offset,y-patch_w+1,stride):
                ...
                if patch_d.shape == (5,5,3) and patch_l.shape == (40, 40, 3):
...
if __name__=='__main__':
    ...
    parser.add_argument('--scale', dest='scale', help='Scaling Factor for Super-Resolution', type=int, default=8)
    parser.add_argument('--labelpath', dest='labelpath', help='Path to HR images (./DIV2K_train_HR)',default='/hdd/tianchuan/DataSet/DIV2K/DIV2K_train_HR')
    parser.add_argument('--datapath', dest='datapath', help='Path to LR images (./DIV2K_train_LR_bicubic)',default='/hdd/wuchengxu/wcx/SR/DataSet/DIV2K_train_LR_bicubic')
    parser.add_argument('--tfrecord', dest='tfrecord', help='Save path for tfrecord file', default='/hdd/tianchuan/Meteorological_data/train_SR_x8')
    ...
    generate_TFRecord(datapath, labelpath, tfrecord_file,5*scale,5*scale,160)
    print('Done')